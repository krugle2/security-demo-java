...
export default class WordCloudMain {
  constructor(el) {
    ...
    $.postWithPrefix(`${this.ajax_url}/get_state`, null, response => {
  ...
  submitAnswer() {
    ...
    // be fired on server's response.
    $.postWithPrefix(`${this.ajax_url}/submit`, $.param(data), response => {
      if (response.status !== 'success') {
        return
      }
  ...
  drawWordCloud(response, words, bounds) {
    ...
    $.each(response.student_words, (word, stat) => {
      ...
      studentWordsKeys.push(
        HtmlUtils.interpolateHtml('{listStart}{startTag}{word}{endTag}{percent}{listEnd}', {
    ...
    HtmlUtils.setHtml(cloudSectionEl.find('.your_words'), HtmlUtils.HTML(studentWordsStr))
    HtmlUtils.setHtml(
      cloudSectionEl.find('.your_words').end().find('.total_num_words'),
      HtmlUtils.interpolateHtml(gettext('{start_strong}{total}{end_strong} words submitted in total.'), {
    ...
    const groupEl = d3
      .select(`#${cloudSectionEl.attr('id')} .word_cloud`)
      .append('svg')
      ...
      .attr('aria-describedby', () =>
        HtmlUtils.interpolateHtml(gettext('text_word_{uniqueId} title_word_{uniqueId}'), {
    ...
    groupEl
      .append('title')
      .attr('id', () =>
        HtmlUtils.interpolateHtml(gettext('title_word_{uniqueId}'), {
    ...
    groupEl
      .append('text')
      .attr('id', () =>
        HtmlUtils.interpolateHtml(gettext('text_word_{uniqueId}'), {
          uniqueId: generateUniqueId(cloudSectionEl.attr('id'), $(this).parent().data('id')),
        })
      )
